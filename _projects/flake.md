---
layout: project
title: Flake
description: Flake is a minimalist and pretty blog theme for Jekyll
thumbnail: /assets/img/flake-portfolio-thumbnail.png
links:
  - label: Download
    url: https://github.com/astreuw/flake
    icon: fa-solid fa-download
  - label: View Code
    url: https://github.com/astreuw/flake
    icon: fa-solid fa-code
badges:
  - https://img.shields.io/github/stars/astreuw/flake
---

# Introduction

Flake is a minimalist blog theme for Jekyll created by <a href="https://astreuw.github.io/about" class="link-raw">Ricardo Costa (@astreuw)</a>, made for those who want a pretty yet simple blog — fast and featureful.

# Features

- Blog features: write, customize and organize your ideas and publications;
- Projects: share your projects and create a custom page to show the world what you've done;
- Custom elements: customize your publications with custom elements from Flake;
- Themes: choose between light and dark theme;
- Syntax highlighting: add life to your embedded code;

# Screenshots

Here are a couple of Flake's screenshots:
![](/assets/img/post/welcome-to-flake/1.png)
![](/assets/img/post/welcome-to-flake/2.png)
![](/assets/img/post/welcome-to-flake/3.png)

# License

© 2026 Ricardo Costa. Distributed under the MIT License.
